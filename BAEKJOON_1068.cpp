...
다른 풀이(모범답안) dfs로 첫번째 노드부터 탐색한다. 이때 자식을 벡터형으로 받는다.
벡터크기가 0이면 leaf노드 이므로 더한다.
이때 del_node가 현 node이면 더 이상 진행하지 않고 -1을 리턴한다. 
이때 이 부모노드가 -1을 받았고, 부모의 밑에 -1만 있었다면 부모가 새로운 leaf노드가 되므로 leaf노드에 +1을 한다.
*/
#include <iostream>
#include <cstring>
